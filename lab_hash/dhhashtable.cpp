...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    unsigned fIndex = hashes::hash(key, size);
    unsigned step = hashes::secondary_hash(key, size);

    while (should_probe[fIndex]){
      fIndex = (fIndex + step) % size;
    }

    should_probe[fIndex] = true;
    table[fIndex] = new std::pair<K, V>(key, value);
    /**
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
    unsigned fIndex = hashes::hash(key, size);
    unsigned step = hashes::secondary_hash(key, size);
    unsigned currStep = fIndex;

    if (should_probe[fIndex] && table[fIndex]->first == key)
      return fIndex;
    else
      currStep = (currStep + step) % size;

    while (currStep != fIndex){
      if (should_probe[currStep] && table[currStep]->first == key)
        return currStep;
      else
        currStep = (currStep + step) % size;
