...
size_t binarySearch(const std::vector<T>& elements, const C& val, size_t indLow, size_t indHigh){
    size_t mid = (indLow+indHigh)/2;
    if (elements[mid] < val && elements[mid + 1] > val){
      return mid + 1;
    }
    else if (elements[mid] > val && elements[mid - 1] < val){
      return mid;
    }
    else if (elements[mid] < val){
      return binarySearch(elements, val, mid + 1, indHigh);
    }
    else if (elements[mid] > val){
      return binarySearch(elements, val, indLow, mid);
    }
    else {
      return mid;
    }
