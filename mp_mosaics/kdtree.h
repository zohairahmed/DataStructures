...
class KDTree
{
  ...
  private:
    ...
    int part(vector<Point<Dim>>& points, int lBound, int pivot, int rBound,
             int dCount);

    Point<Dim> quick(vector<Point<Dim>>& points, int lBound, int k, int rBound,
             int dCount);

    void makeTree(vector<Point<Dim>>& points, int lBound, int rBound, int dCount,
                  KDTreeNode *& sroot);

    void clear(KDTreeNode *& kdtn);

    void copy_(const KDTree<Dim> &other);

    void copyNodes(const KDTreeNode * sroot, KDTreeNode *& toCopy);
    /**
